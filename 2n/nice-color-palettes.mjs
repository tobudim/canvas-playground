/**
 * Get nice color palettes extracted from github.com/Jam3/nice-color-palettes
 * -> Array
 */
export default function getNiceColorPalette() {
  const palettes = [
  ...
  const { length } = palettes;
  const randomPalette = palettes[Math.floor(Math.random() * length)];
